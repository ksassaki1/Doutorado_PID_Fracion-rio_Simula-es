...
[gBestIAE, ~] = optimizeFitness(@calcularIAE, zn_values, nIndividuos, nIteracoes, dimensoes, variancia, limites, G);
[gBestISE, ~] = optimizeFitness(@calcularISE, zn_values, nIndividuos, nIteracoes, dimensoes, variancia, limites, G);


% Calcular IAE e ISE para os melhores parâmetros encontrados após otimização para IAE
IAE_IAE = calcularIAE(gBestIAE, G);
ISE_IAE = calcularISE(gBestIAE, G);

% Calcular IAE e ISE para os melhores parâmetros encontrados após otimização para ISE
IAE_ISE = calcularIAE(gBestISE, G);
ISE_ISE = calcularISE(gBestISE, G);
...
respostaIAE = step(T_IAE, linspace(0, 300, 1000));
...
respostaISE = step(T_ISE, linspace(0, 300, 1000));
