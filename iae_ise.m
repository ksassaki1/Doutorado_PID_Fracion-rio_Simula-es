...
[gBest_ISE, gBestScore_ISE] = otimizarPSO(@calcularISE, G, nParticulas, nIteracoes, dimensoes, limites, valoresIniciais, varianciaInicial);

% Otimização usando IAE
[gBest_IAE, gBestScore_IAE] = otimizarPSO(@calcularIAE, G, nParticulas, nIteracoes, dimensoes, limites, valoresIniciais, varianciaInicial);
...
IAE_ISE = calcularIAE(gBest_ISE, G);
ISE_ISE = calcularISE(gBest_ISE, G);
ITAE_ISE = calcularITAE(gBest_ISE, G);

% Calcule IAE, ISE e ITAE para a sintonia baseada em IAE
IAE_IAE = calcularIAE(gBest_IAE, G);
ISE_IAE = calcularISE(gBest_IAE, G);
ITAE_IAE = calcularITAE(gBest_IAE, G);
...
IAE_ZN = calcularIAE([Kp_ZN, Ki_ZN, Kd_ZN, 1, 1], G);

% Calcular ISE para Ziegler-Nichols
ISE_ZN = calcularISE([Kp_ZN, Ki_ZN, Kd_ZN, 1, 1], G);

% Calcular ITAE para Ziegler-Nichols
ITAE_ZN = calcularITAE([Kp_ZN, Ki_ZN, Kd_ZN, 1, 1], G);
